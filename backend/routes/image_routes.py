...
def create_image_blueprint():
        ...
        try:
            ...
            if thumbnail:
                ...
                if os.path.exists(thumb_filepath):
                    resp = send_file(thumb_filepath, mimetype='image/png')
                    resp.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
                    return resp
                    response = send_file(thumb_filepath, mimetype="image/png")
            ...
            resp = send_file(filepath, mimetype='image/png')
            resp.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
            return resp
    ...
    def text2img():
        ...
        try:
            ...
            history_service.update_record(
                record_id,
                images={"task_id": task_id, "generated": [filename]},
                status="completed",
                thumbnail=filename
            )
            ...
            logger.info(f"✅ 快速生图完成: record={record_id}, task={task_id}")
