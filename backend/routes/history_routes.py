...
def create_history_blueprint():
    ...
    def create_history():
        ...
        try:
            ...
            record_type = data.get('type', 'rednote')
            ...
            record_id = history_service.create_record(topic, outline, task_id, record_type, prompt)
    ...
    def list_history():
        ...
        try:
            ...
            history_service = get_history_service()
            result = history_service.list_records(page, page_size, status, record_type)
    ...
    def download_history_zip(record_id):
        ...
        try:
            ...
            if not record:
                return jsonify({"success": False, "error": f"历史记录不存在：{record_id}"}), 404

            task_id = record.get('images', {}).get('task_id')
            if not task_id:
                return jsonify({"success": False, "error": "该记录没有关联的任务图片"}), 404
            ...
            if record.get('content'):
                c = record['content']
                if c.get('titles'):
                    text_lines.append('=== 标题 ===')
                    text_lines.extend(c['titles'])
                    text_lines.append('')
                if c.get('copywriting'):
                    text_lines.append('=== 文案 ===')
                    text_lines.append(c['copywriting'])
                    text_lines.append('')
                if c.get('tags'):
                    text_lines.append('=== 标签 ===')
                    text_lines.append(', '.join(c['tags']))
                    text_lines.append('')
            text_content = chr(10).join(text_lines)

            zip_buffer = _create_images_zip(task_dir, text_content)
            ...
            return jsonify({"success": False, "error": f"下载失败。\n错误详情: {str(e)}"}), 500

    return history_bp


def _create_images_zip(task_dir: str, text_content: str = '') -> io.BytesIO:
    memory_file = io.BytesIO()

    with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
        # 添加文案 txt
        if text_content:
            zf.writestr('文案内容.txt', text_content.encode('utf-8'))
