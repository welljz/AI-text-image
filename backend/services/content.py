...
class ContentService:
    ...
    def _parse_content_response(self, response_text: str) -> Dict[str, Any]:
        ...
        # 尝试 JSON 格式（旧，向后兼容）
        result = self._parse_legacy_json_response(response_text)
        if result:
            return result

        logger.error(f"无法解析响应: {response_text[:200]}...")
        raise ValueError("AI 返回的内容格式不正确，无法解析为 XML 或 JSON")

    def _parse_xml_response(self, response_text: str) -> Optional[Dict[str, Any]]:
        """解析 XML 格式：<titles> <copywriting> <tags>"""
        def extract(tag, text):
            m = re.search(rf'<{tag}>\s*(.+?)\s*</{tag}>', text, re.DOTALL)
            return m.group(1).strip() if m else None

        titles_raw = extract('titles', response_text)
        copywriting = extract('copywriting', response_text)
        tags_raw = extract('tags', response_text)

        if not titles_raw or not copywriting:
            return None  # XML 不完整，回退
        ...
        logger.info(f"XML 解析成功: {len(titles)} 个标题, {len(tags)} 个标签")
        return {"titles": titles, "copywriting": copywriting, "tags": tags}

    def _parse_legacy_json_response(self, response_text: str) -> Optional[Dict[str, Any]]:
        """向后兼容旧的 JSON 格式"""
        try:
    ...
    def generate_content(
        ...
        try:
            ...
            prompt = self.prompt_template.format(
                topic=topic,
                outline=outline,
                cover_visual=cover_visual if cover_visual else "（封面尚未生成，待生成后更新）"
            )
            ...
            # 解析 JSON 响应
            content_data = self._parse_content_response(response_text)
