...
class HistoryService:
    ...
    def create_record(
        ...
        Args:
            ...
            record_type: 记录类型 "rednote" 或 "quick"（可选，默认 rednote）
            prompt: 快速生图的原始提示词（仅 quick 类型使用）
        ...
        # quick 类型没有 page_count
        if record_type != "quick":
            idx_entry["page_count"] = len(outline.get("pages", []))
        index["records"].insert(0, idx_entry)
    ...
    def list_records(
        ...
        Args:
            ...
            record_type: 类型过滤（可选），支持：rednote/quick
        ...
        # 按类型过滤（未指定类型时默认只显示小红书的）
        if record_type:
            records = [r for r in records if r.get("type", "rednote") == record_type]
