...
class OutlineService:
    ...
    def _parse_outline(self, outline_text: str) -> List[Dict[str, Any]]:
        # 尝试解析 JSON 格式（新格式）
        json_pages = self._try_parse_json_outline(outline_text)
        if json_pages is not None:
            return json_pages

        # 向后兼容：旧的 <page> 分隔符格式
        return self._parse_legacy_outline(outline_text)

    def _try_parse_json_outline(self, outline_text: str) -> Optional[List[Dict[str, Any]]]:
        try:
        ...
        if pages:
            logger.info(f"JSON 格式解析成功，{len(pages)} 页，含 visual_prompt: {sum(1 for p in pages if p.get('visual_prompt'))} 页")
            return pages
        return None

    def _parse_legacy_outline(self, outline_text: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"旧格式解析: {len(pages)} 页")
        return pages
    ...
    def generate_outline(
        ...
        try:
            ...
            # 注入页数要求到主题中
            topic_with_count = f"{topic}\n（需要生成 {page_count} 页）" if page_count else topic
            if style:
                topic_with_count += f"\n【重要】指定的视觉风格：{style}"
            prompt = self.prompt_template.format(topic=topic_with_count, style=style or "无特定要求")
