...
class ImageService:
    ...
    def __init__(self, provider_name: str = None):
        ...
        self.prompt_template_verbose = self._load_prompt_template(verbose=True)
        self.prompt_template_cover = self._load_prompt_template(name="image_prompt_cover.txt")
        self.prompt_template_content = self._load_prompt_template(name="image_prompt_content.txt")
    ...
    def _load_prompt_template(self, short: bool = False, verbose: bool = False, name: str = None) -> str:
        """加载 Prompt 模板"""
        if name:
            filename = name
        elif verbose:
            filename = "image_prompt_verbose.txt"
        elif short:
    ...
    def _build_image_prompt(self, text_content: str, page_type: str, style: str = "", full_outline: str = "", provider_type: str = "") -> str:
        if not style:
            style = "简约现代"

        # 封面和内容页使用不同的 prompt 模板
        if page_type == "cover":
            lines = [l.strip() for l in text_content.split('\n') if l.strip()]
            short_text = '\n'.join(lines[:2]) if lines else text_content[:60]
            if self.prompt_template_cover:
                return self.prompt_template_cover.format(
                    style=style,
            ...
            return self.prompt_template.format(
                ...
                text_content=short_text,
                page_type=page_type,
                full_outline=full_outline[:600] if full_outline else ""
            )
        else:
            ...
            if self.prompt_template_content:
                return self.prompt_template_content.format(
                    style=style,
                    text_content=full_text,
                    page_type=page_type
                )
            if provider_type == "google_genai":
                return self.prompt_template_verbose.format(
                    visual_prompt=style,
                    text_content=full_text,
                    page_type=page_type,
                    full_outline=full_outline[:600] if full_outline else ""
    ...
    def _save_image(self, image_data: bytes, filename: str, task_dir: str = None) -> str:
        ...
        # 生成缩略图（200KB以内，保证预览清晰度）
        thumbnail_data = compress_image(image_data, max_size_kb=200)
    ...
    def generate_text2img(self, prompt: str, task_id: str = None, task_dir: str = None, aspect_ratio: str = "1:1", quality: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ 快速生图成功: {task_id}/{filename}")
            return {"success": True, "filename": filename, "task_id": task_id}
    ...
    def generate_images(
        ...
        # 清理超出当前页数的旧图片
        max_idx = total - 1
        for old_file in os.listdir(self.current_task_dir):
            if old_file.endswith('.png') and not old_file.startswith('thumb_'):
                try:
                    idx = int(old_file.replace('.png', ''))
                    if idx > max_idx:
                        os.remove(os.path.join(self.current_task_dir, old_file))
                        thumb = os.path.join(self.current_task_dir, 'thumb_' + old_file)
                        if os.path.exists(thumb):
                            os.remove(thumb)
                        logger.info(f'  清理孤儿图片: {old_file}')
                except ValueError:
                    pass
    ...
    def retry_single_image(
        ...
        if task_id in self._task_states:
            ...
            if not style:
                style = task_state.get("style", "")
            user_images = task_state.get("user_images")

        if not page.get("style"):
            page["style"] = style
