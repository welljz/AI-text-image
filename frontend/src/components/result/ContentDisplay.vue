...
onMounted(() => { if (store.content.status === "idle" && store.images.length > 0) { handleGenerate() } })
onUnmounted(() => {
...
async function handleGenerate() {
  ...
  try {
    ...
    if (result.success && result.titles && result.copywriting && result.tags) {
      ...
      if (store.recordId) {
        try { await updateHistory(store.recordId, { content: { titles: result.titles, copywriting: result.copywriting, tags: result.tags } }) } catch (e) {}
      }
    } else {
